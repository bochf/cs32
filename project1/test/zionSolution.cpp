...
string Player::takeComputerChosenTurn() {
  ...
  for (int k = 1; k < m_arena->rows() || k < m_arena->cols(); k++) {
    ...
    if (m_row + k <= m_arena->rows() &&
        m_arena->nRobotsAt(m_row + k, m_col) > 0) {
    ...
    if (m_col + k <= m_arena->cols() &&
        m_arena->nRobotsAt(m_row, m_col + k) > 0) {
...
bool Player::shoot(int dir) {
  ...
  do {
    ...
    c += cDelta;
    distance++;
  } while (r >= 1 && r <= m_arena->rows() && c >= 1 && c <= m_arena->cols() &&
           distance <= MAXSHOTLEN);
  return false;
}
